    ...
    void *ptr)
{
    ...
    if (rc == 0 || rc == MQTT_CODE_ERROR_TIMEOUT || rc == MQTT_CODE_WANT_READ
...
#ifdef WOLFMQTT_DEBUG_SOCKET
    if (rc != 0 && rc != MQTT_CODE_WANT_WRITE) { /* hide in non-blocking case */
...
#ifdef WOLFMQTT_DEBUG_SOCKET
    if (rc != 0 && rc != MQTT_CODE_WANT_READ) { /* hide in non-blocking case */
...
exit:
    ...
    if (rc != MQTT_CODE_SUCCESS) {
        ...
        if (client->tls.ssl) {
            ...
            else if (errnum == WOLFSSL_ERROR_WANT_WRITE) {
                return MQTT_CODE_WANT_WRITE;
            }
        #ifdef WOLFMQTT_DEBUG_SOCKET
